...
import { getRef, firstCharUpper, getHumpNaming } from '../utils';

export default (schema: Schema, commentType: CommentType, level = 1, types: EnumTypes, serviceName: string, interfacesEnumTypes?: EnumTypes): InterfaceModel => {
    const relatedInterfaceNames = new Set<string>();
    const EnumStringTypes = new Set<string>();
        ...
        switch (type) {
            ...
            case Type.string:
                if (isEnum(schema)) {
                    ...
                    if (propKey) {
                        TypeKey = getHumpNaming(`${serviceName}${firstCharUpper(propKey)}`.trim())
                        while(types[TypeKey]) {
                            if (types[TypeKey] !== typeContent) {
                                TypeKey = `${TypeKey}_zhh`;
                                types[TypeKey] = typeContent;
                                interfacesEnumTypes && (interfacesEnumTypes[TypeKey] = typeContent);
                            } else {
                                break;
                            }
                        }
    ...
    return {
        ...
        EnumStringTypes: [...EnumStringTypes]
    };
};
