...
module.exports = function(rootPath, configPath) {
  const {port, target, mockApis} = require(configPath);
  ...
  const server = http.createServer(function(req, res) {
    ...
    // 截取 请求的 url 
    const requestUrl = (req.url || api).split('?')[0];
    !isOptions && mockApis && mockApis.forEach(api => {
      if (requestUrl === api) {
        
        console.log(req.method ,method, `${method.toLowerCase()}Paths`)
        ...
        paths && Object.keys(paths).forEach(pathRegStr => {
          // 正则匹配 url
          if (new RegExp(pathRegStr).test(requestUrl)) {
        ...
        responseMockModel && Object.values(responseMockModel).forEach(propValue => {
          ...
          if (type === 'ref') {
            mockSchema[key] = [];
            mockSchema[key][0] = {};
            Object.values(parser.mockDataSchema[ref]).forEach(valU => {
              if (valU.type === 'ref') {
                // 只递归生成一层数据
                mockSchema[key][0][valU.key] = [];
                mockSchema[key][0][valU.key][0] = {};
                Object.values(parser.mockDataSchema[ref]).forEach(val => {
                  if (val.type === 'ref') {
